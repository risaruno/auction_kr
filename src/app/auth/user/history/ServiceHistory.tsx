...
const serviceHistory = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadElectronicIdentityDocument(selectedApplication.id, file)
  ...
  const handleFileDownload = () => {
    if (selectedApplication?.electronic_identity_document_url) {
      ...
      window.open(selectedApplication.electronic_identity_document_url, '_blank')
    }
  }
            ...
            <Box sx={{ p: 2 }}>
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, mb: 3 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant='body1' sx={{ mb: 2 }}>
                    {selectedApplication.case_number || selectedApplication.court_case_number || '-'}
                  </Typography>
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography variant='subtitle1' sx={{ fontWeight: 'bold', color: 'primary.main' }} gutterBottom>
              ...
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, mb: 3 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant='body1' sx={{ mb: 2 }}>
                    {selectedApplication.bid_amount ? formatCurrency(selectedApplication.bid_amount) : '-'}
                  </Typography>
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Typography variant='subtitle1' sx={{ fontWeight: 'bold', color: 'primary.main' }} gutterBottom>
                  ...
                  <Typography variant='body1' sx={{ mb: 2 }}>
                    {selectedApplication.bid_date ? formatDate(selectedApplication.bid_date) : '-'}
                  </Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ mb: 3 }}>
                <Typography variant='subtitle1' sx={{ fontWeight: 'bold', color: 'primary.main', mb: 2 }}>
                  전자본인서명확인서
                </Typography>                <Card variant="outlined" sx={{ mb: 2 }}>
                  <CardContent>
                    {selectedApplication.electronic_identity_document_url ? (
                      <Box>
                        ...
                        <Typography variant='body1' sx={{ fontWeight: 'medium' }}>
                          {selectedApplication.electronic_identity_document_name || '전자본인서명확인서'}
                        </Typography>
                        <Typography variant='caption' color="text.secondary">
                          업로드일: {formatDate(selectedApplication.electronic_identity_document_uploaded_at)}
                        </Typography>
                      </Box>
                    ) : (
                      <Typography variant='body2' color="text.secondary">
                  ...
                  <CardActions>
                    {selectedApplication.electronic_identity_document_url ? (
                      <Button
                        ...
                        size="small"
                        variant="outlined"
                        href={selectedApplication.electronic_identity_document_url}
                        target="_blank"
                ...
                </Card><Typography variant='caption' color="text.secondary">
